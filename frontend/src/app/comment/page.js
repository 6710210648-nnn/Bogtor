  ...
  return (
    <header style={{ backgroundColor: "#fff", display: "flex", alignItems: "center", padding: "10px 40px", boxShadow: "0 1px 6px rgb(0 0 0 / 0.1)", position: "sticky", top: 0, zIndex: 10 }}>
      ...
      <nav style={{ marginLeft: "auto", display: "flex", gap: "24px", fontWeight: "600", fontSize: "16px", color: "#555" }}>
        ...
        <a href="/rating-dashboard" style={{ ...navLinkStyle, color: "#e85d04", borderBottom: "2px solid #e85d04" }}>บทความ</a>
...
function StarDisplay({ comments }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
      <span>
        {[1, 2, 3, 4, 5].map((i) => (
          <span key={i} style={{ color: i <= avg ? "#f5a623" : "#ccc", fontSize: "1.1rem" }}>★</span>
        ))}
      </span>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: "4px", marginBottom: "12px" }}>
      ...
      {[1, 2, 3, 4, 5].map((i) => (
        <span key={i} onClick={() => onChange(i)} onMouseEnter={() => setHovered(i)} onMouseLeave={() => setHovered(0)}
          style={{ fontSize: "1.8rem", color: i <= (hovered || value) ? "#f5a623" : "#ccc", cursor: "pointer", userSelect: "none" }}>
          ★
        </span>
      ))}
...
export default function CommentPage() {
        ...
        <div className="container py-5">
          ...
          <div className="d-flex gap-3 mb-5 flex-wrap align-items-center">
            ...
            <button
              ...
              onClick={() => { setEditArticle(null); setView("form"); }}
            >
              + เขียนบทความใหม่
            </button>
          ...
          {!loading && filtered.map((article) => (
            <div className="row mb-5 pb-5 border-bottom" key={article.id}>
              ...
              <div className="col-md-6 mb-4">
                <div
                  style={{ width: "100%", height: "360px", borderRadius: "16px", overflow: "hidden", boxShadow: "0 4px 15px rgba(0,0,0,0.1)", background: "#f8f0e8", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
              ...
              <div className="col-md-6">
                ...
                <div className="mb-2">
                  <StarDisplay comments={article.comments} />
                </div>
                <p style={{ fontSize: "0.85rem", color: "#888", marginBottom: "1rem" }}>
                  ✍️ {article.author} · 📅 {formatDate(article.createdAt)}
                </p>
                ...
                <div className="d-flex gap-2">
                  ...
                  <button className="btn btn-outline-secondary btn-lg rounded-pill"
                    onClick={() => { setEditArticle(article); setView("form"); }}>
                    ✏️ แก้ไข
                  </button>
                </div>
                <p className="text-muted mt-3" style={{ fontSize: "0.85rem" }}>
                  💬 {(article.comments || []).length} ความเห็น
                </p>
              </div>
            </div>
          ))}
  ...
  // ================= VIEW: ฟอร์มเขียน/แก้ไข =================
  if (view === "form") {
    return (
      <FormView
        editArticle={editArticle}
        onBack={() => { setEditArticle(null); setView("list"); }}
        onSave={handleSaveArticle}
      />
    );
  }

  // ================= VIEW: อ่านบทความ + คอมเม้น =================
  if (view === "detail" && selectedArticle) {
    return (
      <DetailView
        article={selectedArticle}
        onBack={() => setView("list")}
        onEdit={() => { setEditArticle(selectedArticle); setView("form"); }}
...
function FormView({ editArticle, onBack, onSave }) {
  ...
  return (
    <>
      ...
      <div className="container py-5">
        ...
        <h2 className="fw-bold mb-1">{isEdit ? "✏️ แก้ไขบทความ" : "📝 เขียนบทความอาหาร"}</h2>
        <p className="text-muted mb-5">{isEdit ? "แก้ไขข้อมูลบทความของคุณ" : "แชร์ประสบการณ์อาหารให้คนหาดใหญ่รู้จัก"}</p>
            ...
            <div className="card border-0 rounded-4 p-4 mb-4" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              ...
              <div className="mb-3">
                ...
                <textarea className={`form-control ${errors.content ? "is-invalid" : ""}`} rows={8}
                  placeholder="เขียนรีวิวหรือเรื่องราวเกี่ยวกับอาหารหาดใหญ่..." value={content}
              ...
              <p className="text-muted" style={{ fontSize: "0.85rem" }}>
                ⭐ คะแนนดาวจะคำนวณจากผู้ที่มาคอมเม้นโดยอัตโนมัติ
              </p>
            </div>
          </div>
            ...
            <div className="card border-0 rounded-4 p-4" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.08)" }}>
              ...
              {displayImage && (
                <div style={{ position: "relative", marginBottom: "1rem", borderRadius: "12px", overflow: "hidden" }}>
                  ...
                  <button
                    style={{ position: "absolute", top: "8px", right: "8px", background: "rgba(0,0,0,0.6)", color: "#fff", border: "none", borderRadius: "50%", width: "30px", height: "30px", cursor: "pointer" }}
                    onClick={() => { setUploadPreview(null); setImageUrl(""); if (fileInputRef.current) fileInputRef.current.value = ""; }}>
                    ×
                  </button>
                </div>
              )}
              ...
              <input type="text" className="form-control mb-3" placeholder="https://example.com/food.jpg"
...
function DetailView({ article: initArticle, onBack, onEdit, onDelete, onUpdate }) {
      ...
      <div className="container py-5">
        ...
        <div className="d-flex justify-content-between align-items-center mb-5 flex-wrap gap-2">
          ...
          <div className="d-flex gap-2">
            ...
            <button className="btn btn-outline-danger rounded-pill fw-bold" onClick={() => onDelete(article.id)}>🗑️ ลบ</button>
          </div>
        </div>
          ...
          <div className="col-md-6">
            ...
            {avgRating && (
              <p style={{ fontSize: "0.85rem", color: "#888", marginBottom: "4px" }}>
                ⭐ คะแนนเฉลี่ย <strong>{avgRating}</strong>/5 จาก {ratings.length} รีวิว
              </p>
            )}
        ...
        <div className="mt-5">
          <h4 className="fw-bold mb-4">
            💬 ความเห็น
            <span className="ms-2 badge rounded-pill bg-primary" style={{ fontSize: "0.85rem" }}>
                ...
                <div key={c.id} className="card border-0 rounded-4 p-3" style={{ boxShadow: "0 2px 10px rgba(0,0,0,0.07)" }}>
                  ...
                  {c.rating > 0 && (
                    <div style={{ marginBottom: "4px" }}>
                      {[1, 2, 3, 4, 5].map((i) => (
                        <span key={i} style={{ color: i <= c.rating ? "#f5a623" : "#ccc", fontSize: "0.95rem" }}>★</span>
                      ))}
                    </div>
                  )}
